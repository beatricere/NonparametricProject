...
# This is the script that was used to test most of the models. 
# Each model has its own section and the final section shows the model that was 
# chosen applied to the test set. 
...
# Perform stratified split of test set (e.g. 70% training, 30% calibration)
split2 <- initial_split(test, prop = 0.7, strata = bacteremia)

#Extract training and testing datasets for conformal
test.train.set <- training(split2)
test.cal.set <- testing(split2)
...
roc_score
plot(roc_score ,main ="ROC curve test set - GAM with FS using DSAE delta = 0.9")
...
# On the test set with the final model  --------------------------------------------------------
...
pred_probs <- predict(gam_model_fwd, newdata = test, type = 'response')
...
conf_matrix <- confusionMatrix(factor(pred_class,levels = c(0, 1)), factor(test$bacteremia, levels = c(0, 1)))
print(conf_matrix)

conf_matrix <- table(factor(pred_class, levels = c(0, 1)), factor(test$bacteremia, levels = c(0, 1)))
print(conf_matrix)
...
LAC <- function(X){
  P_true <- predict(model, newdata=X, type='response')
  LAC.byclass <- as.matrix(cbind(P_true, 1 - P_true))
  return (LAC.byclass)
}
...
n.test <- dim(test)[1]
test.LAC.scores <- LAC(test)
conformal.test.set <- test.LAC.scores <= qhat.alpha
conformal.test.set

# Verify the conformal region guarantee
# We need to pick the true label of the observations that were classified 
# as class 0 by the conformal classifier
class0.conf.pred.idxs <- as.numeric(which(conformal.test.set[,1] == TRUE))
class0.true.labels <- test[class0.conf.pred.idxs,]$bacteremia
class0.true.labels

class1.conf.pred.idxs <- as.numeric(which(conformal.test.set[,2] == TRUE))
class1.true.labels <- test[class1.conf.pred.idxs,]$bacteremia 
class1.true.labels

# Conformal region guarantee: is the coverage of the conformal sets greater or 
# equal than 1 - alpha?
numerator <- sum((class0.true.labels == FALSE)) + sum((class1.true.labels == TRUE))
denominator <- length(class0.true.labels) + length(class1.true.labels)
...
# Confusion matrix
true.labels <- test$bacteremia  

# We want to remove instances where the model is undecided
undecided <- which(conformal.test.set[,1] == FALSE & conformal.test.set[,2] == FALSE)
conformal.test.set <- conformal.test.set[-undecided,]
true.labels <- true.labels[-undecided]

# Predicted labels: classify as class 1 if conformal.test.set[,2] is TRUE, otherwise class 0
predicted.labels <- ifelse(conformal.test.set[,2] == TRUE, 1, 0)
